...
- The spec defines `zk_credential` (v0.1.0), body transport, public outputs, key rotation via `kid`, and wire encoding for commitments and signatures. Servers can expose issuer keys at `/.well-known/zk-credential-keys` (HTTP GET) to support rotation; deployments may also provision keys out-of-band.
